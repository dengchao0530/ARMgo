...
//			StepCtr(5);//步进电机工作
			...
			switch(mu)
			{
				...
				case 2://累计时间清零
					...
					Display_Timing(0,0,Engineering_Pattern.Led_Display_Timing_Enbale);
					Display_Concentration(0,0);
					Display_CumulativeTime(RunData.CumulativeTime/60,1,Engineering_Pattern.Led_Display_CumulativeTime_Enbale);//暂时分钟为单位
				break;	
					
				case 3://版本号
					Display_Concentration(0,0);
					Display_Timing(VERSION,4,Engineering_Pattern.Led_Display_Timing_Enbale);
					Display_CumulativeTime(0,0,Engineering_Pattern.Led_Display_CumulativeTime_Enbale);//暂时分钟为单位
				break;	
			}				
